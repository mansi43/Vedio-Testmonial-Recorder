...
app.post("/upload", upload.single("video"), async (req, res) => {
  ...
  try {
    ...
    const auth = new google.auth.GoogleAuth({
      keyFile: "video-testimonial-tool-429519-8805dfcd35e5.json", // Path to the service account key file
    ...
    const fileMetadata = {
      ...
      parents: ["1ghkLmVC934fwHA64C8zbCG_JngJ_RUTI"], // Parent folder ID in Google Drive
    };
    const media = {
...
app.listen(PORT, () => {
  ...
  console.log(`Server running on port http://localhost:${PORT}`); // Log the port the server is running on
});
